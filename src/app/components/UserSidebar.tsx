...
const userNavItems: NavItem[] = [
  { label: 'Home', path: '/user/home', icon: <Home className="h-5 w-5" /> },
  { label: 'Book Court', path: '/user/booking', icon: <Calendar className="h-5 w-5" /> },
  { label: 'Subscription', path: '/user/subscription', icon: <CreditCard className="h-5 w-5" /> },
  { label: 'History', path: '/user/history', icon: <History className="h-5 w-5" /> },
  { label: 'Contact', path: '/user/contact', icon: <Phone className="h-5 w-5" /> },
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-72 flex-col border-r border-emerald-100/80 bg-gradient-to-b from-[#eef6e8] via-[#f8fbf4] to-[#edf3e8] shadow-[4px_0_32px_rgba(45,74,34,0.08)] lg:flex">
      ...
      <div className="relative flex h-full flex-col px-4 py-5">
        <Link to="/user/home" className="mb-6 flex items-center gap-3 rounded-2xl border border-white/70 bg-white/70 p-3 shadow-sm backdrop-blur-sm">
              ...
              <Link key={item.path} to={item.path} className={navItemClass(active)}>
                ...
                <ChevronRight
                  className={`h-4 w-4 shrink-0 transition-transform ${
                    active ? 'translate-x-0.5 text-[#808000]' : 'text-gray-300 group-hover:translate-x-0.5 group-hover:text-[#808000]'
                  }`}
                />
              </Link>
            );
