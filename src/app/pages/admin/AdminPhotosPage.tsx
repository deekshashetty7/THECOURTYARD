...
const uploadImageToCloudinary = async (dataUrl: string, fileName: string) => {
  ...
  const response = await fetch(`${getApiBaseUrl()}/admin/gallery/upload`, {
    method: 'POST',
    headers: {
...
export const AdminPhotosPage = () => {
  ...
  useEffect(() => {
    ...
    const loadGalleryFromApi = async () => {
      try {
        const res = await fetch(`${getApiBaseUrl()}/gallery`, { cache: 'no-store' });
        if (!res.ok) return;
    ...
    try {
      ...
      if (draft.dataUrl.startsWith('data:image/')) {
        try {
          ...
          const uploadMessage = uploadError instanceof Error ? uploadError.message : 'Unable to upload image';
          console.error('Cloudinary upload failed:', uploadError);
          throw new Error(`Image upload failed: ${uploadMessage}. Please try again in a moment.`);
        }
      }
