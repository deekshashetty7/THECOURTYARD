...
function describeConnectionTargetSafely() {
  if (env.mysqlUrl) {
    try {
      const parsed = new URL(env.mysqlUrl);
      const database = parsed.pathname ? parsed.pathname.replace(/^\//, '') : '(unknown)';
      return `MYSQL_URL -> host=${parsed.hostname} port=${parsed.port || '(default)'} database=${database}`;
    } catch {
      return 'MYSQL_URL is set but could not be parsed as a valid URL';
    }
  }

  return `individual vars -> host=${env.mysqlHost || '(empty)'} port=${env.mysqlPort || 3306} database=${env.mysqlDatabase || '(empty)'} user=${env.mysqlUser || '(empty)'}`;
}
...
function getPool() {
  if (!pool) {
    ...
    console.log(`[mysqlServices] Connecting to database using ${describeConnectionTargetSafely()}`);
